...
def config_aaa_server(params, server_type):
    ...
    if deadtime is not None:
        cmds.append(f"{server_type}-server deadtime {deadtime}")

    if server_timeout is not None:
        cmds.append(f"{server_type}-server timeout {server_timeout}")

    if directed_request is not None:
        if directed_request == "enabled":
            cmds.append(f"{server_type}-server directed-request")
        elif directed_request == "disabled":
            cmds.append(f"no {server_type}-server directed-request")

    if global_key is not None:
        cmds.append(f"{server_type}-server key {encrypt_type} {global_key}")
...
def default_aaa_server(existing, params, server_type):
    ...
    if global_key is not None and existing_key is not None:
        cmds.append(f"no {server_type}-server key 7 {existing_key}")
...
def main():
    argument_spec = {
        "server_type": {"type": "str", "choices": ["radius", "tacacs"], "required": True},
        "global_key": {"type": "str", "no_log": True},
        "encrypt_type": {"type": "str", "choices": ["0", "7"]},
        "deadtime": {"type": "str"},
        "server_timeout": {"type": "str"},
        "directed_request": {"type": "str", "choices": ["enabled", "disabled", "default"]},
        "state": {"choices": ["default", "present"], "default": "present"},
    }
