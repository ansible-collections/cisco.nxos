...
def remove_filter(host, udp, proposed, existing):
    commands = []
    if existing.get("vrf_filter") and proposed.get("vrf_filter") in existing.get("vrf_filter"):
        commands.append(
            "no snmp-server host {} filter-vrf \
                    {} udp-port {}".format(
                host,
...
def remove_src(host, udp, proposed, existing):
    ...
    if existing.get("src_intf"):
        commands.append(
            "no snmp-server host {} source-interface \
                    {} udp-port {}".format(
                host,
...
def main():
    argument_spec = {
        "snmp_host": {"required": True, "type": "str"},
        "community": {"type": "str"},
        "udp": {"type": "str", "default": "162"},
        "version": {"choices": ["v1", "v2c", "v3"]},
        "src_intf": {"type": "str"},
        "v3": {"choices": ["noauth", "auth", "priv"]},
        "vrf_filter": {"type": "str"},
        "vrf": {"type": "str"},
        "snmp_type": {"choices": ["trap", "inform"]},
    ...
    if version is None:
        version = existing.get("version") if existing else "v1"

    if snmp_type is None:
        snmp_type = existing.get("snmp_type") if existing else "trap"

    if v3 is None and version == "v3" and existing:
        v3 = existing.get("v3")
    ...
    if not any([vrf_filter, vrf, src_intf]) and not all([snmp_type, version, community, udp]):
        module.fail_json(
