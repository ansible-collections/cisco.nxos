...
def parse_facility_link_status(line, facility, status):
    ...
    if facility is not None:
        match = re.search(
            r"logging level {0} {1} (\S+)".format(facility, status),
            line,
            re.M,
        )
        if match:
...
def parse_event_status(line, event):
    ...
    match = re.search(
        r"logging event {0} (\S+)".format(event + "-status"),
        line,
        re.M,
    )
    if match:
...
def parse_message(line):
    ...
    match = re.search(
        r"logging message interface type ethernet description",
        line,
        re.M,
    )
    if match:
...
def parse_file_size(line, name, level):
    ...
    match = re.search(
        r"logging logfile {0} {1} size (\S+)".format(name, level),
        line,
        re.M,
    )
    if match:
...
def parse_dest_level(line, dest, name):
    ...
    if dest and dest != "server":
        if dest == "logfile":
            match = re.search(
                r"logging logfile {0} (\S+)".format(name),
                line,
                re.M,
            )
            if match:
...
def parse_facility_level(line, facility, dest):
    ...
    if dest == "server":
        ...
        match = re.search(
            r"logging level {0} (\S+)".format(facility),
            line,
            re.M,
        )
        if match:
...
def parse_use_vrf(line, dest):
    ...
    if dest and dest == "server":
        match = re.search(
            r"logging server (?:\S+) (?:\d+) use-vrf (\S+)",
            line,
            re.M,
        )
        if match:
