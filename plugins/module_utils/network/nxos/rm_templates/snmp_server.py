    ...
    PARSERS = [
        ...
        },
        {
            ...
            "setval": ''' snmp-server community {{ name }} {{ (' group ' + group) if group is defined else '' }} 
                          snmp-server community {{ name }} {{ ' ro' if ro|d(False) else ''}} {{ ' rw' if rw|d(False) else ''}}
                          snmp-server community {{ name }} {{ (' use-ipv4acl ' + use_ipv4acl) if use_ipv4acl is defined else '' }} {{ (' use-ipv6acl ' + use_ipv6acl) if use_ipv6acl is defined else '' }}
                          ''' ,
